...
[
  {
    ...
    "cmd": {
      "development": [
        "underpost state set container-status dd-myapp-app-instance-1-development-initializing-deployment",
        "node server.js"
      ],
      "production": [
        "underpost state set container-status dd-myapp-app-instance-1-production-initializing-deployment",
        "node server.js"
      ]
    },
    "volumes": [
...
An instance config may declare K8S-native lifecycle hooks and probes. These splice directly into the container spec generated by `instance-build-manifest` / `instance`, **replacing the anti-pattern of chaining `underpost state set container-status …` calls inside the shell `cmd` string**.

```jsonc
{
  "id": "mmo-server",
  "runtime": "cyberia-server",
  "image": "underpost/cyberia-server:v3.4.5",
  "fromPort": 8081,
  "toPort": 8081,
  "cmd": {
    "production": [
      ...
      "postStart": {
        "exec": {
          "command": [
            "sh",
            "-c",
            "underpost state set container-status dd-cyberia-mmo-server-production-initializing-deployment || true",
          ],
        },
      },
      "preStop": {
        "exec": {
          "command": [
            "sh",
            "-c",
            "underpost state set container-status dd-cyberia-mmo-server-production-stopping-deployment || true",
          ],
        },
      },
    },
...
```jsonc
{
  ...
  "lifecycle": {
    "development": {
      ...
      "postStart": {
        "exec": { "command": ["sh", "-c", "underpost state set container-status …-initializing-deployment || true"] },
      },
      "preStop": {
        "exec": { "command": ["sh", "-c", "underpost state set container-status …-stopping-deployment || true"] },
      },
    },
    "production": {
      "imagePullPolicy": "Always",
...
- name: dd-cyberia-mmo-server-production-blue
  ...
  …
  lifecycle:
    postStart: { exec: { command: ["sh", "-c", "underpost state set container-status …-initializing-deployment || true"] } }
    preStop:   { exec: { command: ["sh", "-c", "underpost state set container-status …-stopping-deployment || true"] } }
```
...
[
  {
    ...
    "cmd": {
      "development": [
        "underpost state set container-status dd-mywebapp-frontend-development-initializing-deployment",
        "exec node server.js"
      ],
      "production": [
        "underpost state set container-status dd-mywebapp-frontend-production-initializing-deployment",
        "node server.js"
      ]
    },
    "volumes": [
  ...
  },
  {
    ...
    "cmd": {
      "development": [
        "underpost state set container-status dd-mywebapp-api-development-initializing-deployment",
        "set -a && . /var/run/secrets/mywebapp/api/development.env && set +a && export ENGINE_GRPC_ADDRESS={{grpc-service-dns}} && node dist/main.js"
      ],
      "production": [
        "underpost state set container-status dd-mywebapp-api-production-initializing-deployment",
...
Both Dockerfiles install `underpost@${UNDERPOST_VERSION}` at build time (`ARG UNDERPOST_VERSION=3.4.5`), so the `cmd` entries can call `underpost state set container-status …` without paying an `npm install -g` cost on every pod boot. The two CI workflows that publish these images forward `UNDERPOST_VERSION` as a build-arg to keep the in-image CLI aligned with the image tag.

Both instances also set `lifecycle.<env>.imagePullPolicy: "Always"` so kubelet re-pulls the published image on every rollout — the CI tags (`v3.4.5`, `v3.4.5`, …) move forward only through coordinated bumps, but the registry image itself can be republished under the same tag during incident response, and `Always` ensures the cluster picks that up. See [imagePullPolicy — per-instance override](#imagepullpolicy--per-instance-override-extension).
