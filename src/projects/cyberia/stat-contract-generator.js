...
// Pinned so every checkout compiles byte-identical Go protobuf output.
const PROTO_TOOLS = {
  buf: 'github.com/bufbuild/buf/cmd/buf@v1.72.0',
  plugins: ['google.golang.org/protobuf/cmd/protoc-gen-go@v1.36.12', 'google.golang.org/grpc/cmd/protoc-gen-go-grpc@v1.6.1'],
};

export function buildProtoContractArtifacts() {
  const source = readFileSync(new URL('../../grpc/cyberia/cyberia.proto', import.meta.url), 'utf8');
  const proto = '// Code generated by Cyberia stat-contract from engine src/grpc/cyberia/cyberia.proto. DO NOT EDIT.\n\n' + source;
  const template = JSON.stringify({
    version: 'v2',
    plugins: PROTO_TOOLS.plugins.map((plugin) => ({ local: ['go', 'run', plugin], out: 'out', opt: ['paths=source_relative'] })),
  });
  const temporary = mkdtempSync(join(tmpdir(), 'cyberia-proto-contract-'));
  try {
    mkdirSync(join(temporary, 'in'));
    writeFileSync(join(temporary, 'in', 'cyberia.proto'), proto);
    execFileSync('go', ['run', PROTO_TOOLS.buf, 'generate', 'in', '--template', template], { cwd: temporary, stdio: 'inherit', timeout: 600000 });
    const compiled = readdirSync(join(temporary, 'out')).sort();
    return {
      'cyberia-server/gen/proto/cyberia.proto': proto,
      ...Object.fromEntries(compiled.map((name) => ['cyberia-server/gen/proto/' + name, readFileSync(join(temporary, 'out', name), 'utf8')])),
    };
  } finally {
    rmSync(temporary, { recursive: true, force: true });
  }
...
export async function generateStatContract({ root = fileURLToPath(new URL('../../../', import.meta.url)), check = false } = {}) {
  const artifacts = { ...buildStatContractArtifacts(), ...buildBootContractArtifacts(), ...buildProtoContractArtifacts() };
